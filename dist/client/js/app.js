...
window.fetch = async function() {
    ...
    // Allow calls to auth endpoints without token
    if (typeof resource === 'string' && resource.includes('/api/auth/')) {
        return originalFetch(resource, config);
    }
    
    const token = localStorage.getItem('girly_token');
    if (!token && resource.includes('/api/')) {
    ...
    if (response.status === 401 && resource.includes('/api/')) {
        localStorage.removeItem('girly_token');
        window.location.href = '/login';
    }
    return response;
};
...
// Check auth on load
if (!localStorage.getItem('girly_token') && window.location.pathname !== '/login') {
    window.location.href = '/login';
}
...
function initGreeting() {
  ...
  let greet = 'ðŸŒ™ Good night,';
  if (hour >= 5 && hour < 12) greet = 'ðŸŒ… Good morning,';
  else if (hour >= 12 && hour < 17) greet = 'â˜€ï¸ Good afternoon,';
  else if (hour >= 17 && hour < 21) greet = 'ðŸŒ† Good evening,';

  const el = document.getElementById('greeting');
  if (el) el.textContent = greet + ' Queen! ðŸ‘‘';
...
function switchView(view) {
  ...
  const emojis = { dashboard: 'âœ¨', students: 'ðŸ’–', 'add-student': 'ðŸŒ¸' };
  const labels = { dashboard: 'Dashboard', students: 'My Students', 'add-student': 'Add Student' };

  const titleEl = document.getElementById('pageTitle');
  if (titleEl) titleEl.innerHTML = `<span class="title-emoji">${emojis[view] || 'âœ¨'}</span> ${labels[view] || ''}`;
...
async function loadDashboard() {
  try {
    ...
    if (d.topStudent && d.topStudent.id) {
      ...
      if (card) {
        card.style.display = '';
        setText('spotlightEmoji', d.topStudent.profile_emoji || 'ðŸ‘©â€ðŸŽ“');
        setText('spotlightName', `${d.topStudent.first_name} ${d.topStudent.last_name}`);
        setText('spotlightDetails', `${d.topStudent.course} Â· ${d.topStudent.status}`);
...
function renderCourseChart(data) {
  ...
  if (!el) return;
  if (!data || data.length === 0) { el.innerHTML = emptyChart('No courses yet ðŸ“š'); return; }
...
function renderMoodChart(data) {
  ...
  if (!el) return;
  if (!data || data.length === 0) { el.innerHTML = emptyChart('No moods recorded yet ðŸ˜Š'); return; }
...
function renderGenderChart(data, total) {
  ...
  if (!el) return;
  if (!data || data.length === 0 || total === 0) { el.innerHTML = emptyChart('No gender data yet ðŸŒˆ'); return; }
...
async function loadRecentStudents() {
  try {
    ...
    if (!el) return;
    if (recent.length === 0) { el.innerHTML = emptyChart('No students yet ðŸŒ·'); return; }
    el.innerHTML = recent.map(s => `
      <div class="recent-student-row">
        <div class="avatar-emoji" style="background:linear-gradient(135deg,${s.favorite_color||'#ff6b9d'}22,${s.favorite_color||'#a855f7'}22)">
          ${s.profile_emoji || 'ðŸ‘©â€ðŸŽ“'}
        </div>
        <div class="recent-student-info">
          <h4>${s.first_name} ${s.last_name}</h4>
          <p>${s.course} Â· GPA ${parseFloat(s.gpa).toFixed(2)} Â· ${s.mood || 'ðŸ˜Š'}</p>
        </div>
...
function renderTable() {
  ...
  tbody.innerHTML = students.map(s => {
    ...
    <tr data-id="${s.id}">
      ...
      <td><span class="student-mood">${s.mood || 'ðŸ˜Š'}</span></td>
...
function populateCourseFilter() {
  ...
  sel.innerHTML = '<option value="">ðŸ’ All Courses</option>' +
...
async function editStudent(id) {
  try {
    ...
    document.getElementById('mood').value        = s.mood || 'ðŸ˜Š';
    document.getElementById('profileEmoji').value = s.profile_emoji || 'ðŸ‘©â€ðŸŽ“';

    // Update pickers visually
    setActivePicker('emojiPicker', 'emoji-btn', 'data-emoji', s.profile_emoji || 'ðŸ‘©â€ðŸŽ“');
    setActivePicker('moodPicker',  'mood-btn',  'data-mood',  s.mood || 'ðŸ˜Š');

    document.getElementById('formTitle').innerHTML = `<span class="form-title-emoji">âœï¸</span> Edit Student`;
...
function resetForm() {
  ...
  document.getElementById('profileEmoji').value = 'ðŸ‘©â€ðŸŽ“';
  document.getElementById('mood').value = 'ðŸ˜Š';
  document.getElementById('favoriteColor').value = '#ff69b4';
  document.getElementById('colorLabel').textContent = '#ff69b4';
  document.getElementById('formTitle').innerHTML = `<span class="form-title-emoji">ðŸŒ¸</span> Add New Student`;
  document.getElementById('submitBtn').innerHTML  = `<i class="fas fa-wand-magic-sparkles"></i> Save Student`;
  setActivePicker('emojiPicker', 'emoji-btn', 'data-emoji', 'ðŸ‘©â€ðŸŽ“');
  setActivePicker('moodPicker',  'mood-btn',  'data-mood',  'ðŸ˜Š');
  clearErrors();
}
...
function showToast(message, type = 'info') {
  ...
  const icons = { success: 'ðŸŽ‰', error: 'ðŸ˜¢', info: 'ðŸ’­' };
...
function logout() { localStorage.removeItem('girly_token'); window.location.href = '/login'; }
